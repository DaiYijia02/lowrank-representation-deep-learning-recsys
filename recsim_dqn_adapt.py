...
from rep_ucb_agent import RepUCBAgent
...
class ResponseAdapter(object):
    ...
    def encode(self, responses):
        response_tensor = np.zeros(
            self._response_shape, dtype=self._response_dtype)
...
class RepUCBAgentRecSim(RepUCBAgent):
    """RecSim-specific Dopamine DQN agent that converts the observation space."""

    def __init__(self, _output_slatesess, observation_space, num_actions, stack_size,
