...
class Runner(object):
    ...
    def _log_one_step(self, user_obs, doc_obs, slate, responses, reward,
        ...
        for i, doc in enumerate(list(doc_obs.values())):
            ...
            _add_float_feature(
                fl['doc_%d' % i], spaces.flatten(doc_space, doc))
    ...
    def _run_one_episode(self):
        ...
        while True:
            ...
            if done:
                break
            elif rand_gamma < 1 - self._agent.gamma:
                break
        ...
        self._agent._store_transition(
            observation, self._agent.action, reward, True)

        # sample two uniform action data points
        for i in range(2):
            action = np.random.randint(self._agent.num_actions)
            observation, reward, done, info = self._env.step(action)
            self._agent._store_transition(observation, action, reward, True)
        ...
        return step_number, total_reward  # not including uniform sampling
    ...
    def _write_metrics(self, step, suffix):
        ...
        add_summary('AverageEpisodeLength', np.mean(
            self._stats['episode_length']))
        add_summary('AverageEpisodeRewards', np.mean(
            self._stats['episode_reward']))
...
class EvalRunner(Runner):
    ...
    def __init__(self,
        ...
        self._checkpoint_dir = os.path.join(
            train_base_dir, 'train', 'checkpoints')
