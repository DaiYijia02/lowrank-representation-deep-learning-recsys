...
import recsim_env.dynamic as dynamic
...
@gin.configurable
class EstimatedUser(user.User):
    """User learned by representation model."""
    ...
    def next_response(self, slate_docs):
        choice_guess = slate_docs.get('choice_guess')
        # user_vec = slate_docs.get('uservec_guess')
        consume_time = slate_docs.get('consumedtime_guess')
        return Value(consumed_time=consume_time[0]).union(Value(choice_guess=choice_guess))
        # return Value(consumed_time=[consume_time]).union(Value(choice_guess=[choice_guess]))
    
    def specs(self):
        state_spec = ValueSpec()
        response_spec = ValueSpec(
            consumed_time=tensor_space(
                low=0.0, high=np.Inf, shape=(self._num_users,))).union(ValueSpec(
            choice_guess=tensor_space(
                    low=0.0,
                    high=self._slate_size, shape=(self._num_users,))))
...
class InterestEvolutionUser(user.User):
    ...
    def __init__(
        ...
        # choice_model_ctor=selector_lib.ArgmaxChoiceModel,
    ...
    def next_state(self, previous_state, user_response,
        ...
        # Debug: step-wise user_vec and slate_doc
        # tf.print(user_interests)
        # tf.print(slate_docs.get('doc_features'))
        # tf.print(chosen_docs)
