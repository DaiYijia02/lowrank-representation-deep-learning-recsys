...
class UniformRandomChoiceModel(ChoiceModel):
  """A uniform random choice model.

  Samples item x in scores according to
     p(x) = 1 / total number of items

  Attributes:
    num_choices: slate size.
    num_docs: total number of items.
  ...
  def choice(self, slate_document_logits):
    ...
    slate_document_logits0 = tf.concat(
        (slate_document_logits, self._nochoice_logits),
        axis=-1)
    num_docs = tf.shape(slate_document_logits0)[-1]

    def sampling_fn(logit_tensor):
        del logit_tensor
        logit_tensor = tf.constant(np.log([[1/self._num_docs for i in range(self._num_docs)]+[0]]), dtype=tf.float32)
    ...
    joint = tfd.JointDistributionCoroutine(lambda: sampling_fn(slate_document_logits0))
...
class MultinomialLogitChoiceModel(ChoiceModel):
  ...
  def choice(self, slate_document_logits):
    ...
    slate_document_logits0 = tf.concat(
        ...
        axis=-1)
    print("-----------------------------------------")
    with tf.compat.v1.Session() as sess: print(slate_document_logits0.eval())
    return Value(
...
class ArgmaxChoiceModel(ChoiceModel):
  """An argmax choice model.
  """
  ...
  def choice(self, slate_document_logits):
    ...
    slate_document_logits0 = tf.concat(
        ...
        axis=-1)
    return Value(
        choice=tf.cast(tf.math.argmax(slate_document_logits0,1), tf.int32))
...
# class IteratedArgmaxChoiceModel(ChoiceModel):
#   """An argmax choice model.
#   """
