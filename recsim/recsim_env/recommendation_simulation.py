...
def estimated_recs_story(
  ...
  # Construct entities.
  user = user_ctor(config, phi, mu, actor, critic)
  user_spec = user.specs()
  corpus = corpus_ctor(config)
  corpus_spec = corpus.specs()
  recommender = recommender_ctor(config, phi, mu, actor, critic)
  ...
  user_observation.value = variable.value(user.observation)
  ...
  return variables, recommender._phi_model, recommender._mu_model, recommender._actor_model, recommender._critic_model

def recs_story_for_representation(
    config,
    user_ctor,
    corpus_ctor,
    recommender_ctor,
