...
from rlpyt_buffer import AsyncPrioritizedSequenceReplayFrameBufferExtended, \
    AsyncUniformSequenceReplayFrameBufferExtended
from contrastive_ucb_models import from_categorical, to_categorical
...
class SPRCategoricalDQN(CategoricalDQN):
    ...
    def __init__(self,
        ...
        self.bonus_return_ = np.zeros((16, 32))  # TODO
    ...
    def initialize_replay_buffer(self, examples, batch_spec, async_=False):
        ...
        if self.prioritized_replay:
            ...
            buffer = AsyncPrioritizedSequenceReplayFrameBufferExtended(
                **replay_kwargs)
        else:
            buffer = AsyncUniformSequenceReplayFrameBufferExtended(
                **replay_kwargs)
    ...
    def optim_initialize(self, rank=0):
        ...
        if self.bonus_flag:
            if self.count_action:
                self.Lambda_total = torch.diag(torch.zeros(
                    self.agent.n_atoms)+1e-5).to(self.agent.device)
            else:
                self.Lambda_total = torch.diag(torch.zeros(
                    self.agent.n_atoms*self.model.num_actions)+1e-5).to(self.agent.device)
    ...
    def optimize_agent(self, itr, samples=None, sampler_itr=None):
        ...
        opt_info = ModelOptInfo(
            *([] for _ in range(len(ModelOptInfo._fields))))
        if itr < self.min_itr_learn:
            return opt_info

        # sample. all_observation.shape=[sequence_length, batch_size, framestack,
        # image_shape0, imageshape1], all_reward.shape=[sequence_length, batch_size],
        # return_.shape=[sequence_length-n_step_return, batch_size],
        # done.shape=[sequence_length-n_step_return, batch_size],
        # done_n.shape=[sequence_length-n_step_return, batch_size].
        samples_from_replay = self.replay_buffer.sample_batch(
            self.batch_size)
        if self.bonus_flag:
            with torch.no_grad():
                B1, B2 = samples_from_replay.all_observation.squeeze(
                ).shape[:2]
                state_size = samples_from_replay.all_observation.squeeze(
                ).shape[-3:]  # [framestack, image_shape0, imageshape1]
                obs_reshape = samples_from_replay.all_observation.squeeze().reshape(
                    B1*B2, *state_size)  # [framestack, image_shape0, imageshape1]
                obs_reshape_transform = self.agent.model.transform(
                    obs_reshape, augment=False)  # [framestack, image_shape0, imageshape1]
                obs_features = self.agent.model.stem_forward(obs_reshape_transform.to(
                    self.agent.device))   # conv features (512, 64, 7, 7)
                obs_features = self.agent.model.head_forward(
                    obs_features, None, None, False)           # (512, num_action, 51)
                assert (obs_features >= 0.0).all()

                if self.count_action:       # extract features of the action
                    # (512, 51). after select the action
                    obs_features = select_at_indexes(samples_from_replay.all_action.reshape(
                        B1*B2).to(self.agent.device), obs_features)

                obs_features = obs_features.reshape(B1*B2, -1)
                # print("** obs_features:", obs_features.shape)
                # (512, 51*n_action, 51*n_action)
                Lambda_iter = torch.matmul(
                    obs_features.unsqueeze(-1), obs_features.unsqueeze(1))
                # print("** Lambda_iter:", Lambda_iter.shape)
                Lambda_iter_sum = torch.sum(
                    Lambda_iter, dim=0)         # (306, 306)

                # calculate the bonus
                self.Lambda_total += Lambda_iter_sum      # update the global Lambda matrix
                # (512, 1, 51*n_action) * (51*n_action, 51*n_action) = (512, 1, 51*n_action)
                bonus_phi_lambda = torch.matmul(
                    obs_features.unsqueeze(1), torch.inverse(self.Lambda_total))
                ...
                # (512, 1, 51*n_action) * (512, 51*n_action, 1) = (512, 1, 1)
                bonus_phi_lambda_phi = torch.matmul(
                    bonus_phi_lambda, obs_features.unsqueeze(-1))
                ...
                bonus_sqrt = torch.sqrt(
                    bonus_phi_lambda_phi).squeeze()     # (512,)
                # print("isnan sqrt:", torch.isnan(bonus_sqrt).any())
                bonus_sqrt = torch.nan_to_num(
                    bonus_sqrt, nan=0.0)          # nan to zero
                # assert not torch.isnan(bonus_sqrt).any()

                bonus_weight = self.bonus_factor * \
                    bonus_sqrt               # (512,)
                bonus_final = bonus_weight.reshape(
                    B1, B2).cpu().numpy()    # (B1, B2)  (16, 32)
                ...
                for n1 in range(0, B1-self.n_step_return):
                    for n2 in range(0, self.n_step_return):
                        return_[n1, :] += (self.discount ** n2) * bonus_final[n1+n2, :] * (
                            1 - samples_from_replay.done.cpu().numpy()[n1, :])
            ...
            total_loss = loss + self.model_rl_weight * \
                model_rl_loss + self.reward_loss_weight*reward_loss
            ...
            # grad_norm is a float sometimes, so wrap in tensor
            ...
            if self.bonus_flag:
                opt_info.bonus.append(
                    self.bonus_return_[0].mean())   # TODO: log
            # opt_info.bonus
    ...
    def dist_rl_loss(self, log_pred_ps, samples, index):
        ...
        # [P']      shape=[51,], discount=0.99, n_step_return=10
        next_z = z * (self.discount ** self.n_step_return)
        # [B,P']    (32, 51)
        next_z = torch.ger(1 - samples.done_n[index].float(), next_z)
        # [B,1]     (32, 1)  index处的样本
        ret = samples.return_[index].unsqueeze(1)
        ...
        next_z = torch.clamp(ret + next_z, self.V_min,
                             self.V_max)     # [B,P']    (32, 51)

        # [1,P,1]
        z_bc = z.view(1, -1, 1)
        next_z_bc = next_z.unsqueeze(
            1)                                # [B,1,P']
            ...
            if self.double_dqn:
                next_ps = self.agent(samples.all_observation[index + self.n_step_return],
                                     samples.all_action[index +
                                                        self.n_step_return],
                                     samples.all_reward[index + self.n_step_return])       # [B,A,P']
                next_qs = torch.tensordot(
                    next_ps, z, dims=1)                              # [B,A]
                # [B]
                next_a = torch.argmax(next_qs, dim=-1)
            else:
                target_qs = torch.tensordot(
                    target_ps, z, dims=1)        # [B,A]
                next_a = torch.argmax(target_qs, dim=-1)                 # [B]
            target_p_unproj = select_at_indexes(
                next_a, target_ps)       # [B,P']
            target_p_unproj = target_p_unproj.unsqueeze(
                1)               # [B,1,P']
            target_p = (target_p_unproj *
                        projection_coeffs).sum(-1)     # [B,P]
        p = select_at_indexes(
            samples.all_action[index + 1].squeeze(-1), log_pred_ps.cpu())  # [B,P]
        # p = torch.clamp(p, EPS, 1)  # NaN-guard.
        losses = -torch.sum(target_p * p, dim=1)             # Cross-entropy.

        target_p = torch.clamp(target_p, EPS, 1)
        KL_div = torch.sum(
            target_p * (torch.log(target_p) - p.detach()), dim=1)
        # Avoid <0 from NaN-guard.
    ...
    def loss(self, samples):
        ...
        rl_loss, KL = self.rl_loss(
            log_pred_ps[0], samples, 0)   # 调用 dqn_rl_loss
        if len(pred_rew) > 0:
            pred_rew = torch.stack(pred_rew, 0)
            with torch.no_grad():
                reward_target = to_categorical(
                    samples.all_reward[:self.jumps+1].flatten().to(self.agent.device), limit=1).view(*pred_rew.shape)
        ...
        if self.model_rl_weight > 0:
            for i in range(1, self.jumps+1):
                jump_rl_loss, model_KL = self.rl_loss(
                    log_pred_ps[i], samples, i)
                model_rl_loss = model_rl_loss + jump_rl_loss

        nonterminals = 1. - \
            torch.sign(torch.cumsum(samples.done.to(
                self.agent.device), 0)).float()
        nonterminals = nonterminals[self.model.time_offset: self.jumps +
                                    self.model.time_offset+1]
